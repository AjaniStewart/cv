/*
  ...
  File: s3.cpp
  Purpose: computes surface normal of object from images
*/
...
Image* compute_normals_in_images(Image* i1, Image* i2, Image* i3, 
          int thresh, int step, d_matrix inverse_source_matrix);
...
{ 
  if (argc != 8) {
    cout << "usage: ./s3 [input_directions] [image 1] [image 2] [image 3] [step] ";
  ...
  try {
    ...
    cout << "invalid value for step: " << e.what() << "\n";
    exit(EXIT_FAILURE);
  }
  ...
  Image* out = compute_normals_in_images(i1,i2,i3,threshold,step,source_inverse);
  
  if (!WriteImage(argv[7],*out)) {
    cout << "could not write image to: " << argv[7] << endl;
    exit(EXIT_FAILURE);
  }

  cout << "wrote image to: " << argv[7] << endl;
  exit(EXIT_SUCCESS);
} 
...
Image* compute_normals_in_images(Image* i1, Image* i2, Image* i3, 
  ...
  d_matrix normals;
  double max_length = 0;
  for (size_t i = 0; i < out->num_rows(); i += step) {
    for (size_t j = 0; j < out->num_columns(); j += step) {
  ...
  for (size_t i = 0; i < coords.size(); ++i) {
    auto unit_normal = compute_unit_vector(normals[i]);
    auto length = (compute_vector_length(normals[i]) / max_length * 15) + 1;

    for (auto& i : unit_normal) {
      i = i * length;
    }
    ...
    //one radius circle
    out->SetPixel(coords[i].first + 1,coords[i].second,255);
    out->SetPixel(coords[i].first - 1,coords[i].second,255);
    out->SetPixel(coords[i].first,coords[i].second + 1,255);
    out->SetPixel(coords[i].first,coords[i].second - 1,255);
    //row = y, col = x
    DrawLine(coords[i].first,coords[i].second,
            coords[i].first + unit_normal[1],
            coords[i].second + unit_normal[0],255,out);

    out->SetPixel(coords[i].first,coords[i].second,0);
    //make into unit
    //divide length by max length
    // multiply by 10
    //add 1
    //draw coords
